...
model feedback()
  // Reactions:
    vMAT: 0.1075 ATP + 0.007278 MALTec -> 0.1075 ADP + 0.1075 PHOS + MALTic;          (p_MAT_VmMAT / (p_MAT_KmMAT_ATP * p_MAT_KmMAT_Maltec)) * (ATP * (MALTec - Maltecmin) / ((1 + ATP / p_MAT_KmMAT_ATP) * (1 + (MALTec - Maltecmin) / p_MAT_KmMAT_Maltec)));
    vMALX2: MALTic -> 2 GLCi;                         p_MALX2_VmMALX2 * MALTic / (p_MALX2_KmMALX2_Maltic + MALTic);
    vINV: SUCec -> GLCec + FRCec;                    p_INV_VmINV * SUCec / (p_INV_KmINV + SUCec);
    FinFructose: -> FRCec;                            Fin * FRCin / Vbroth; 
    FoutFructose: FRCec -> ;                          Fout * FRCec / Vbroth; 
    FinSucrose: -> SUCec;                             Fin * SUCin / Vbroth; 
    FoutSucrose: SUCec -> ;                           Fout * SUCec / Vbroth; 
    FinMaltose: -> MALTec;                            Fin * MALTin / Vbroth; 
    FoutMaltose: MALTec -> ;                          Fout * MALTec / Vbroth; 
   
    # vGLT: 0.007278 GLCec -> GLCi;                     p_GLT_VmGLT * ((GLCec-Csmin)-GLCi / p_GLT_KeqGLT) / (p_GLT_KmGLTGLCo * (1+(GLCec-Csmin)/p_GLT_KmGLTGLCo+GLCi / p_GLT_KmGLTGLCi + 0.91 * (GLCec-Csmin) * GLCi / (p_GLT_KmGLTGLCi * p_GLT_KmGLTGLCo)) + (FRCec - FRCecmin) / p_GLT_KiGLTFRC); 
    vGLT: 0.007278 GLCec -> GLCi;                     p_GLT_VmGLT * ((GLCec-Csmin)-GLCi / p_GLT_KeqGLT) / (p_GLT_KmGLTGLCo * (1+(GLCec-Csmin)/p_GLT_KmGLTGLCo+GLCi / p_GLT_KmGLTGLCi + 0.91 * (GLCec-Csmin) * GLCi / (p_GLT_KmGLTGLCi * p_GLT_KmGLTGLCo) + (FRCec - FRCecmin) / p_GLT_KiGLTFRC));
    # vGLK: GLCi + ATP -> G6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcat * (f_HXK1+f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kglc) * (ATP * GLCi-((ADP * G6P) / p_HXK1_Keq))) / ((1+ATP / p_HXK1_Katp+ADP / p_HXK1_Kadp) * (1 + GLCi / p_HXK1_Kglc+G6P / p_HXK1_Kg6p + T6P / p_HXK1_Kt6p + FRCi / p_HXK1_KiFRC))); 
    vGLK: GLCi + ATP -> G6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcat * (f_HXK1+f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kglc) * (ATP * GLCi-((ADP * G6P) / p_HXK1_Keq))) / ((1+ATP / p_HXK1_Katp+ADP / p_HXK1_Kadp) * (1 + GLCi / p_HXK1_Kglc+G6P / p_HXK1_Kg6p + T6P / p_HXK1_Kt6p + (FRCi / p_HXK1_KiFRC))));
    # vFRT: 0.007278 FRCec -> FRCi;                     p_FRT_VmFRT * ((FRCec - FRCecmin) - FRCi / p_FRT_KeqFRT) / (p_FRT_KmFRTFRCo * (1 + (FRCec - FRCecmin)/p_FRT_KmFRTFRCo + FRCi/p_FRT_KmFRTFRCi + 0.91 * (FRCec - FRCecmin) * FRCi / (p_FRT_KmFRTFRCi * p_FRT_KmFRTFRCo) + ((GLCec - Csmin) / p_FRT_KiFRTGLC)));   
    vFRT: 0.007278 FRCec -> FRCi;                    p_FRT_VmFRT * ((FRCec - FRCecmin) - FRCi / p_FRT_KeqFRT) / (p_FRT_KmFRTFRCo * (1 + (FRCec - FRCecmin)/p_FRT_KmFRTFRCo + FRCi/p_FRT_KmFRTFRCi + 0.91 * (FRCec - FRCecmin) * FRCi / (p_FRT_KmFRTFRCi * p_FRT_KmFRTFRCo) + (GLCec - Csmin)/p_FRT_KiFRTGLC));
    # vFRK: FRCi + ATP -> F6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcatfrc * (f_HXK1 + f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kfrc) * (ATP * FRCi - ((ADP * F6P) / p_HXK1_Keqfrc))) / ((1 + ATP / p_HXK1_Katp + ADP / p_HXK1_Kadp) * (1 + FRCi / p_HXK1_Kfrc + F6P / p_HXK1_Kf6p + T6P / p_HXK1_Kt6p + GLCi / p_HXK1_KiGLC)));
    vFRK: FRCi + ATP -> F6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcatfrc * (f_HXK1 + f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kfrc) * (ATP * FRCi - ((ADP * F6P) / p_HXK1_Keqfrc))) / ((1 + ATP / p_HXK1_Katp + ADP / p_HXK1_Kadp) * (1 + FRCi / p_HXK1_Kfrc + F6P / p_HXK1_Kf6p + T6P / p_HXK1_Kt6p + (GLCi/p_HXK1_KiGLC))));
    # inhib # vGLT: 0.007278 GLCec -> GLCi;                     p_GLT_VmGLT * ((GLCec-Csmin)-GLCi / p_GLT_KeqGLT) / (p_GLT_KmGLTGLCo * (1+(GLCec-Csmin)/p_GLT_KmGLTGLCo+GLCi / p_GLT_KmGLTGLCi + 0.91 * (GLCec-Csmin) * GLCi / (p_GLT_KmGLTGLCi * p_GLT_KmGLTGLCo))); 
    # inhib # vGLK: GLCi + ATP -> G6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcat * (f_HXK1+f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kglc) * (ATP * GLCi-((ADP * G6P) / p_HXK1_Keq))) / ((1+ATP / p_HXK1_Katp+ADP / p_HXK1_Kadp) * (1 + GLCi / p_HXK1_Kglc+G6P / p_HXK1_Kg6p + T6P / p_HXK1_Kt6p))); 
    # inhib # vFRT: 0.007278 FRCec -> FRCi;                     p_FRT_VmFRT * ((FRCec - FRCecmin) - FRCi / p_FRT_KeqFRT) / (p_FRT_KmFRTFRCo * (1 + (FRCec - FRCecmin)/p_FRT_KmFRTFRCo + FRCi/p_FRT_KmFRTFRCi + 0.91 * (FRCec - FRCecmin) * FRCi / (p_FRT_KmFRTFRCi * p_FRT_KmFRTFRCo)));    
    # inhib # vFRK: FRCi + ATP -> F6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcatfrc * (f_HXK1 + f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kfrc) * (ATP * FRCi - ((ADP * F6P) / p_HXK1_Keqfrc))) / ((1 + ATP / p_HXK1_Katp + ADP / p_HXK1_Kadp) * (1 + FRCi / p_HXK1_Kfrc + F6P / p_HXK1_Kf6p + T6P / p_HXK1_Kt6p)));
  ...
  p_GLT_KeqGLT = 1; 
  p_GLT_KmGLTGLCi = 0.0108; 
  p_GLT_KmGLTGLCo = 0.0108; 
  p_GLT_VmGLT = 0.1779; 
  
  p_HXK1_Katp = 0.0545;
  p_HXK1_Kadp = 0.0783;
  p_HXK1_Keq = 18.8164;
  p_HXK1_Kg6p = 2.9537e+03;
  p_HXK1_Kglc = 0.0033;
  p_HXK1_Kt6p = 0.0626;
  p_HXK1_kcat = 6.7325; 
  ...
  p_PFK_Cf26bp = 0.0283; #0.0182; #0.0283; # control here ! 
  ...
  p_TDH1_Keq = 0.0054;
  p_TDH1_Kglyceral3p = 1.6433; #0.5145;
  p_TDH1_Kglycerate13bp = 0.9221; #0.9076;  # control here ! 
  p_TDH1_Knad = 1.2030; #1.1775;  # control here ! 
  p_TDH1_Knadh = 0.0418; #0.0419;  # control here ! 
  p_TDH1_Kpi = 0.8143; #0.7731;  # control here ! 
  p_TDH1_kcat = 60.3784; #78.6422;  # control here ! 
  ...
  p_HOR2_Kglyc3p = 1.5101; #2.5844;  # control here ! ! !
  p_HOR2_Kpi = 2.5491;
  p_HOR2_kcat = 2.2437; #1.2748;  # control here ! ! !
  ...
  p_TPS1_Kg6p = 0.3471;
  p_TPS1_Kudp_glc = 0.1589;
  p_TPS1_kcat = 728.5755;
  p_TPS1_Kpi = 0.4405;
  p_TPS1_KmF6P = 0.0323;
  ...
  p_vacuolePi_k = 0.1699; #0.2619; #0.1699; # control here !!
  ...
  poly_sinkPYR =-0.16132830000000; #0.016132830000000 
  ...
  # parameters from added reactions for uptake and processing of fructose, sucrose and maltose
  p_GLT_KiGLTFRC = 0.0294; 
  p_HXK1_kcatfrc = 34.3035;
  p_HXK1_Kfrc = 0.1215; 
  p_HXK1_Keqfrc = 3.7886; 
  p_HXK1_Kf6p = 3.2050e+03;   
  p_HXK1_KiGLC = 978.8793; 
  
  p_HXK1_KiFRC = 5.6252e+05;
  
  p_MAT_VmMAT = 1.3636; 
  p_MAT_KmMAT_ATP = 8.0385; 
  p_MAT_KmMAT_Maltec = 0.3953; 
  p_nATP_nMaltec = 0.1075;
  p_MALX2_VmMALX2 = 0.1733; 
  p_MALX2_KmMALX2_Maltic = 0.2471; 
  p_FRT_VmFRT = 1.2490;
  p_FRT_KeqFRT = 1; 
  p_FRT_KmFRTFRCo = 2.4522; 
  p_FRT_KmFRTFRCi = 2.4522; 
  p_FRT_KiFRTGLC = 1.1067; 
  p_INV_VmINV = 0.9496; 
  p_INV_KmINV = 0.0989; 
  
  # # fructose setup-specific
  # GLCin = 0;
  # FRCin = 7500 / 180; 
  # SUCin = 0; 
  # MALTin = 0;   
  ...
  # Maltecmin = 0.0149;  
    ...
    # Fout = Fin;
    # glycSynth_K = 0.73 * 0.0021;
    # glycDeg_K = 0.73 * 5.7535e-04;
    # temp_v_glycSynth = 0;
    # temp_v_glycDeg = 0;
    ...
    glycSynth_K = 0;
    glycDeg_K = 0;
    temp_v_glycSynth  := 0.73 * piecewise(sin_y2, time > 61, sin_y1, time > 401, sin_y2, time > 461, sin_y1, time > 801, sin_y2, time > 861, sin_y1, time > 1201, sin_y2, time > 1261, sin_y1, time > 1601, sin_y2, time > 1661, sin_y1, time > 2001, 0);
    temp_v_glycDeg    := 0.73 * piecewise(sin_y2d, time > 31, sin_y1d, time > 401, sin_y2d, time > 431, sin_y1d, time > 801, sin_y2d, time > 831, sin_y1d, time > 1201, sin_y2d, time > 1231, sin_y1d, time > 1601, sin_y2d, time > 1631, sin_y1d, time > 2001, 0);
...
# result = r.simulate(0, 5, 100)
# result = r.simulate(0, 1, 100)
...
# plt.figure()
# temp_id = 27;
# plt.plot(result[:,0],result[:,temp_id],'b-')
