...
model feedback()
  // Reactions:
    ...
    vINV: SUCRec -> GLCec + FRCec;                    p_INV_VmINV * SUCec / (p_INV_KmINV + SUCec);
    ...
    # inhib # vGLT: 0.007278 GLCec -> GLCi;                     p_GLT_VmGLT * ((GLCec-Csmin)-GLCi / p_GLT_KeqGLT) / (p_GLT_KmGLTGLCo * (1+(GLCec-Csmin)/p_GLT_KmGLTGLCo+GLCi / p_GLT_KmGLTGLCi + 0.91 * (GLCec-Csmin) * GLCi / (p_GLT_KmGLTGLCi * p_GLT_KmGLTGLCo)) + (FRCec - FRCecmin) / p_GLT_KiGLTFRC); 
    # inhib # vGLK: GLCi + ATP -> G6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcat * (f_HXK1+f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kglc) * (ATP * GLCi-((ADP * G6P) / p_HXK1_Keq))) / ((1+ATP / p_HXK1_Katp+ADP / p_HXK1_Kadp) * (1 + GLCi / p_HXK1_Kglc+G6P / p_HXK1_Kg6p + T6P / p_HXK1_Kt6p + FRCi / p_HXK1_KiFRC))); 
    # inhib # vFRT: 0.007278 FRCec -> FRCi;                     p_FRT_VmFRT * ((FRCec - FRCecmin) - FRCi / p_FRT_KeqFRT) / (p_FRT_KmFRTFRCo * (1 + (FRCec - FRCecmin)/p_FRT_KmFRTFRCo + FRCi/p_FRT_KmFRTFRCi + 0.91 * (FRCec - FRCecmin) * FRCi / (p_FRT_KmFRTFRCi * p_FRT_KmFRTFRCo) + ((GLCec - Csmin) / p_FRT_KiFRTGLC)));   
    # inhib # vFRK: FRCi + ATP -> F6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcatfrc * (f_HXK1 + f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kfrc) * (ATP * FRCi - ((ADP * F6P) / p_HXK1_Keqfrc))) / ((1 + ATP / p_HXK1_Katp + ADP / p_HXK1_Kadp) * (1 + FRCi / p_HXK1_Kfrc + F6P / p_HXK1_Kf6p + T6P / p_HXK1_Kt6p + GLCi / p_HXK1_KiGLC)));
    ...
    vFRT: 0.007278 FRCec -> FRCi;                     p_FRT_VmFRT * ((FRCec - FRCecmin) - FRCi / p_FRT_KeqFRT) / (p_FRT_KmFRTFRCo * (1 + (FRCec - FRCecmin)/p_FRT_KmFRTFRCo + FRCi/p_FRT_KmFRTFRCi + 0.91 * (FRCec - FRCecmin) * FRCi / (p_FRT_KmFRTFRCi * p_FRT_KmFRTFRCo)));    
    vFRK: FRCi + ATP -> F6P + ADP;                    p_HXK_ExprsCor * (((p_HXK1_kcatfrc * (f_HXK1 + f_HXK2)) / (p_HXK1_Katp * p_HXK1_Kfrc) * (ATP * FRCi - ((ADP * F6P) / p_HXK1_Keqfrc))) / ((1 + ATP / p_HXK1_Katp + ADP / p_HXK1_Kadp) * (1 + FRCi / p_HXK1_Kfrc + F6P / p_HXK1_Kf6p + T6P / p_HXK1_Kt6p)));
  ...
  # Maltecmin = 0.0149;  
    ...
    # Fout = Fin;
    # glycSynth_K = 0.46 * 0.0021;
    # glycDeg_K = 0.46 * 5.7535e-04;
    # temp_v_glycSynth = 0;
    # temp_v_glycDeg = 0;
    ...
    glycSynth_K = 0;
    glycDeg_K = 0;
    temp_v_glycSynth  := 0.46 * piecewise(sin_y2, time > 61, sin_y1, time > 401, sin_y2, time > 461, sin_y1, time > 801, sin_y2, time > 861, sin_y1, time > 1201, sin_y2, time > 1261, sin_y1, time > 1601, sin_y2, time > 1661, sin_y1, time > 2001, 0);
    temp_v_glycDeg    := 0.46 * piecewise(sin_y2d, time > 31, sin_y1d, time > 401, sin_y2d, time > 431, sin_y1d, time > 801, sin_y2d, time > 831, sin_y1d, time > 1201, sin_y2d, time > 1231, sin_y1d, time > 1601, sin_y2d, time > 1631, sin_y1d, time > 2001, 0);
